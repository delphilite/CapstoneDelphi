...
//功能：计算需要覆盖的机器指令大小，借助了 Capstone 反汇编引擎以免指令被从中间切开
//参数：
////////////////////////////////////////////////////////////////////////////////
function CalcHookProcSize(AFunc: Pointer): Integer;
const
{$IFDEF CPUX64}
  defDisasmMode: cs_mode = CS_MODE_64; // for x64 ...
{$ELSE}
  defDisasmMode: cs_mode = CS_MODE_32; // for x32 ...
{$ENDIF}
var
  bEnough: Boolean;
  nAddress: UInt64;
...
begin
  ...
  if AFunc = nil then
    Exit;
  nHandle := 0;
  if cs_open(CS_ARCH_X86, defDisasmMode, nHandle) = CS_ERR_OK then
  try
    pInsn := cs_malloc(nHandle);
    if pInsn <> nil then
    try
      pCode := PByte(AFunc);
      nSize := $20;
      nAddress := NativeUInt(AFunc);
      nRet := 0;
      bEnough := False;
      while cs_disasm_iter(nHandle, pCode, nSize, nAddress, pInsn) do
      begin
        Inc(nRet, pInsn^.size);
        if nRet >= SizeOf(TNewProc) then
        begin
          bEnough := True;
          Break;
        end;
